...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
	...
	std::string tempString = sql.c_str();                       // TEXT VERSION OF sql
	std::string newsql;                                         // SANITIZED SQL STRING
	std::smatch match;                                          // PATTERN MATCH WITHIN STRING
	std::regex numInjection(" or [1-9]+=[1-9]+;");              // PATTERN TO MATCH FOR 'OR 1=1;'
	std::regex wordInjection(" or '[a-zA-Z]+'='[a-zA-Z]+';");   // PATTERN TO MATCH FOR 'OR 'he'='he';'

	// SEARCH FOR / DETECT SQL INJECTION PATTERN MATCH - NUMERIC
	// IF DETECTED, REPLACE THE MATCHING PATTERN WITH A SEMICOLON ';'
	// RERUN THE QUERY USING THE SANITIZED SQL STRING 'newsql'
	if (std::regex_search(tempString, match, numInjection)) {
		std::cout << "\nINJECTION DETECTED: Number Match found! Injected code removed.";
		newsql = std::regex_replace(tempString, numInjection, ";");
		return run_query(db, newsql, records);
	}
	// SEARCH FOR / DETECT SQL INJECTION PATTERN MATCH - ALPHA
	if (std::regex_search(sql, match, wordInjection)) {
		std::cout << "\nINJECTION DETECTED: Word Match found! Injected code removed.";
		newsql = std::regex_replace(tempString, wordInjection, ";");
		return run_query(db, newsql, records);
	}
