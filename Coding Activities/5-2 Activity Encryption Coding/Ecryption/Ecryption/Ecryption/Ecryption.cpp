...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
	...
	{ // TODO: student need to change the next line from output[i] = source[i]
	  // transform each character based on an xor of the key modded constrained to key length using a mod
		// algorithm for encryption of source data
...
std::string read_file(const std::string& filename)
{
	...
	file_text.assign((std::istreambuf_iterator<char>(infile)), std::istreambuf_iterator<char>());
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
	...
	// convert time_t to tm data type so data style required can be extracted
	std::time_t t = std::time(0);
	struct std::tm ltm;
	localtime_s(&ltm, &t);
	// extract year, month, and day from the local time method
	int year = 1900 + ltm.tm_year;
	int month = 1 + ltm.tm_mon;
	int day = ltm.tm_mday;
	// write the required information to the file in specific order
	std::ofstream outfile;
	outfile.open(filename);
	outfile << student_name << std::endl;	// student name
	outfile << year << "-" << month << "-" << day << std::endl;	// timestamp (yyyy-mm-dd)
	outfile << key << std::endl;	// key used
	outfile << data << std::endl;	// data
	outfile.close();
}
